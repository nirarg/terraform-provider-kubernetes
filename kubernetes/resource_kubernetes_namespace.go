...
func resourceKubernetesNamespaceCreate(d *schema.ResourceData, meta interface{}) error {
	...
	out, err := conn.CoreV1().Namespaces().Create(context.Background(), &namespace, metav1.CreateOptions{})
	if err != nil {
		return err
	}
...
func resourceKubernetesNamespaceRead(d *schema.ResourceData, meta interface{}) error {
	...
	namespace, err := conn.CoreV1().Namespaces().Get(context.Background(), name, meta_v1.GetOptions{})
	if err != nil {
...
func resourceKubernetesNamespaceUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	out, err := conn.CoreV1().Namespaces().Patch(context.Background(), d.Id(), pkgApi.JSONPatchType, data, metav1.PatchOptions{})
	if err != nil {
		return err
	}
...
func resourceKubernetesNamespaceDelete(d *schema.ResourceData, meta interface{}) error {
	...
	err = conn.CoreV1().Namespaces().Delete(context.Background(), name, meta_v1.DeleteOptions{})
	if err != nil {
		return err
	}
	...
	stateConf := &resource.StateChangeConf{
		...
		Refresh: func() (interface{}, string, error) {
			out, err := conn.CoreV1().Namespaces().Get(context.Background(), name, meta_v1.GetOptions{})
			if err != nil {
...
func resourceKubernetesNamespaceExists(d *schema.ResourceData, meta interface{}) (bool, error) {
	...
	_, err = conn.CoreV1().Namespaces().Get(context.Background(), name, meta_v1.GetOptions{})
	if err != nil {
		if statusErr, ok := err.(*errors.StatusError); ok && statusErr.ErrStatus.Code == 404 {
			return false, nil
		}
